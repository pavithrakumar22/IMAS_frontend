...
import { CaseData, DiseaseData } from "./types"
...
export function PreviousCasesModal({ cases, onClose }: PreviousCasesModalProps) {
  ...
  const getOutcomeColor = (outcome: string) => {
    switch (outcome) {
      case 'cured': return 'bg-green-100 text-green-800 border border-green-200'
      case 'improved': return 'bg-blue-100 text-blue-800 border border-blue-200'
      case 'referred': return 'bg-yellow-100 text-yellow-800 border border-yellow-200'
      case 'ongoing': return 'bg-gray-100 text-gray-800 border border-gray-200'
  ...
  // FIXED: Handle read full diagnosis click
  ...
  return (
    <>
            ...
            {cases.length === 0 ? (
              ...
              <div className="space-y-8">
                {cases.map((caseData) => {
                  ...
                  return (
                    <div key={caseData.id} className="border border-gray-200 rounded-lg bg-white hover:shadow-lg transition-all">
      ...
      {/* Full Diagnosis Modal - Now properly connected */}
      {selectedDisease && (
        <FullDiagnosisModal
